...
const CASE_STUDY_IDS = [56600, 56559, 56584];
...
export default async function SuccessStoriesSection() {
  const nodes = await getCaseStudyNodes(CASE_STUDY_IDS);
          ...
          {slides.map((slide) => (
            ...
            >
              <figure className="mb-5 md:mb-6 lg:mb-8 xl:mb-10 overflow-hidden">
                <img
                  className="w-full aspect-[3/2] object-cover transition-all duration-500 group-hover:scale-105 ease-out"
                  src={slide.image}
                  alt={slide.imageAlt}
                />
              </figure>
