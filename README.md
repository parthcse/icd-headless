...
**Live domain split** (the site is live): the public front-end is **`www.icecubedigital.com`** — this Next.js app, served by Node (`next start`) on **cPanel**, behind Cloudflare. The WordPress backend, media, and Yoast SEO now live on the **`cms.icecubedigital.com`** subdomain (GraphQL at `cms.icecubedigital.com/graphql`, uploads served from `cms…/wp-content/`).
...
- **Contact form** (`components/common/ContactForm.js`): submits **directly to the live WordPress Contact Form 7** form (id from `NEXT_PUBLIC_CF7_FORM_ID`, default `4751`) at `/wp-json/contact-form-7/v1/contact-forms/<id>/feedback`. Bot protection is **Cloudflare Turnstile** (`NEXT_PUBLIC_TURNSTILE_SITE_KEY`, with a test key on non-production hosts). It also does light geo-IP country prefill.
  ...
  - **Turnstile `600010`** = the challenge couldn't execute client-side. Usually transient (network, device clock skew, privacy extensions). If it's *consistent*, check the widget's **hostname allow-list** in the Cloudflare dashboard — every host the site serves on (`www.`, apex, and any staging host) must be listed.
...
| `loop` | Seamless loop (default `true`). Set `false` when every item must stay tappable — loop clones are `inert` The track then stops at either end. |
| `mobileOnly` | Slider **below 768px only**; from `md` up the stage becomes a CSS grid laid out by `stageClassName` (e.g. `"md:grid-cols-2 lg:grid-cols-3 md:gap-space-small"`), with swipe/autoplay/nav/clones off. Done in CSS (`.carousel-mobile-only` in `styles/input.css`), so there is no layout jump on hydration. Used by the service-page Portfolio + Case Study sections (1 slide at a time; add `slider-nav-on-image` to put the arrows on the card image — see the comment in `styles/input.css`). |
| `stageClassName` | Extra classes on `.owl-stage` — the desktop grid for `mobileOnly`. |
| `navAnchor` | Selector of an element inside each slide (e.g. `"figure"`) to pin the arrows to. The carousel measures it and sets `--nav-y` / `--nav-start` / `--nav-end` on `.owl-nav`; pair with the `slider-nav-on-image` class. Used by the Portfolio + Case Study sliders. |

**Home Creative Services tabs** are React state in [`components/home/ServicesTabs.js`](./components/home/ServicesTabs.js) (content stays in `ServicesSection.js`). They do **not** use `Carousel`: below 768px the strip is a native `overflow-x` scroller (scrollbar hidden via `.no-scrollbar`), because native scrolling follows the finger with the phone's own momentum, while the carousel jumps one slot per swipe. The `animate` reveal classes sit on wrappers React never re-renders — `common-next.js` adds `start` straight to the DOM, and React rewriting that element's `className` on a tab change would drop it and hide the tab.

**Looping is seamless:** the track renders the slides twice and snaps back with the transition disabled once it crosses into the clone set, so autoplay never visibly rewinds. Going **back** from the first slide is two moves: a silent jump to its copy in the clone set, then an animated step back on the next frame (`stepBackRef` in `prev()`). If only the jump happens, the slider looks stuck on slide 1 — that was a real bug, fixed 2026-09-18. That's why the built HTML shows 2× the slide count in `.owl-item` — expected, not a bug.
...
### Static-asset `Cache-Control` (`next.config.mjs` → `headers()`)

Next serves everything in `public/` with `Cache-Control: public, max-age=0` by default. That tells the browser **and** Cloudflare not to cache, so every font, stylesheet and image made a full round trip to the origin — measured at **~1.0 s** from a Singapore Cloudflare edge, for **577 KiB per visitor**.

`headers()` overrides that per directory. The TTLs deliberately differ, because these filenames are **not** content-hashed (unlike `/_next/static/*`, which Next already marks immutable):

| Path | TTL | Why |
| --- | --- | --- |
| `/fonts/:path*` | 1 year, `immutable` | woff2 files are never edited in place |
| `/assets/:path*` | 30 days | images — purge Cloudflare if you replace one keeping its filename |
| `/js/:path*` | 1 day + SWR 1 week | vendor libs are pinned; `common-next.js` / `load-design.js` do get edited |
| `/css/:path*` | 1 hour + SWR 1 day | `output.css` is regenerated by `npm run build:css` and keeps its name |

**Don't raise the CSS/JS TTLs** without content-hashing the filenames first — a long `immutable` on a file that gets overwritten in place strands visitors on a stale copy.

Cloudflare already caches these file types by default; it only needed the origin's permission. Verify after deploying:

```bash
curl -sSI https://www.icecubedigital.com/css/output.css | grep -i 'cache-control\|cf-cache-status'
# want: cf-cache-status: HIT   (first request per edge PoP will legitimately be MISS)
```

### HTML is still not edge-cached
...
- **Network dependency tree** — critical path ~353 ms; Lighthouse explicitly reports no preconnect candidates.
- **`email-decode.min.js`** — Cloudflare Email Address Obfuscation (Scrape Shield), not project code.
- **Legacy JavaScript (~24 KiB)** — 13.5 KiB from browserslist `"> 0.5%"`, 10.5 KiB from Lucky Orange's own bundle. Tightening browserslist drops iOS 15.0–15.3 support to save 13.5 KiB; not a good trade.

---

## Third-party scripts

All live in `components/common/Analytics.js`, rendered from the root layout.

| Script | Strategy | Notes |
| --- | --- | --- |
| Google Tag Manager | `afterInteractive` | Needs to fire early for tracking accuracy |
| Lucky Orange | `lazyOnload` | Legacy pre-2021 snippet, site `73641` |
| Hello Bar | `afterInteractive` | Left early **on purpose** — it injects a bar at the top of the page, so deferring it risks a CLS regression |
| JivoChat | `lazyOnload` | ~18 KiB loader that pulls a much larger chat bundle |
| Hotjar | **disabled** | Commented out — delete the two markers to restore |
| Cloudflare Turnstile | **on form interaction** | See below — deliberately *not* loaded on page view |

**Turnstile loads only when a visitor engages the contact form** (`pointerenter` / `touchstart` / `focus` on the `<form>`, wired in `ContactForm.js`). It is *not* rendered on mount. Two reasons: it costs main-thread time on every page, and its bot-fingerprinting probes WebGL, emitting ~100 `WebGL: INVALID_ENUM` console warnings **per page** — which pile up endlessly under client-side navigation. Viewport-based arming was tried first and was not enough: service pages put the form in the hero banner, so it was always on-screen at load. Hover/tap gives the widget seconds of lead time and the visitor still has a whole form to fill, so a token is ready well before submit; if it somehow isn't, submit already shows "Please complete the security check".

**Not in this repo** (don't go looking for them): **Microsoft Clarity** and Google's `wcm/loader.js` are injected by the **GTM container**; the Cloudflare Insights beacon is auto-injected by Cloudflare. Clarity does the same job as Hotjar, which is why disabling Hotjar didn't leave the site without session recording.

Gotchas:

- **Don't re-add the Lucky Orange tag with site-id `461ec88d`.** `settings.luckyorange.com/461ec88d` returns **404** — it loaded 42 KiB, recorded nothing, and logged a console error on every page view.
- **Lucky Orange's legacy `clickstream.js` opens websockets to `visitors.live`** that fail inside Google's PageSpeed runner. Those hosts resolve fine on a real network, so **real visitors are unaffected** — but it fails the Best Practices "errors in console" audit, and that audit is binary (7 errors fail it exactly as hard as 1).
- **Best Practices 100 is not reachable with this stack.** Third-party cookies (weight 5) fails by definition with any of these scripts, and the Shared Storage API deprecation fires inside Google's own tag. ~90 is the realistic ceiling.

---

## Deployment & operational notes

Deploys to **cPanel** (Node running `next start`) — *not* Vercel. Deploy is `git pull` + `npm run build` on the server. The front-end is **`www.icecubedigital.com`**; the WordPress backend + media + Yoast SEO live on **`cms.icecubedigital.com`**. Both sit behind Cloudflare. Real gotchas:

- **Build-time SEO & content:** metadata, JSON-LD, and the sitemap are generated at **build time** — after any CMS content or SEO change, **rebuild**, or the live site won't reflect it. (This is why the whole live site once showed `noindex`; see [SEO, sitemap & indexing](#seo-sitemap--indexing).)
- **Empty CMS data at build time (403):** Cloudflare **Bot Fight Mode** can challenge the **build server's IP**, so WPGraphQL returns 403 and CMS-backed sections render empty. **Fix:** disable Bot Fight Mode for the GraphQL path, then rebuild.
...
- **Below-the-fold `<img>` needs `loading="lazy"`** — React 19 auto-emits a `<link rel="preload">` for every server-rendered image, which triggers Chrome's "preloaded but not used" warning and wastes bandwidth. This compounds badly with client-side navigation: the console never clears, so every page you visit stacks more warnings.
  - **Keep eager:** anything above the fold — the hero/banner background (`fetchPriority="high"`, it's the LCP element), the header logo, the floating WhatsApp button.
  - **Owl carousels are fine to lazy _if_ CSS reserves the box** (an explicit width or `aspect-[…]`), because the slider then measures correctly whether or not the image has loaded. Only images relying on their *intrinsic* size must stay eager — e.g. `AboutOurCultureSection`, whose `<img>` has no width/aspect, so lazy would give Owl a 0-height stage on init.
- **Nav links (Header/Footer) use `<Link prefetch={false}>`** — internal navigation is `next/link` (client-side, no reload), but the header mega-menu has ~180 links. With default prefetch, Next eagerly prefetches every one on page load, and React 19 injects a `<link rel="preload" as="image">` for each target page's banner image into the *current* head → a flood of "preloaded but not used" warnings on every page. `prefetch={false}` stops the eager prefetch while keeping soft navigation (Next fetches on click instead). **Any new nav/menu link must set `prefetch={false}`.** Content links (home cards, in-body CTAs) keep default prefetch — they're few and curated.
- **Never lengthen the `/css/` or `/js/` cache TTL** in `next.config.mjs` — those filenames aren't content-hashed, so a long `immutable` strands visitors on a stale build. See [Caching & performance](#caching--performance).
- **A PageSpeed console error isn't automatically a real one.** Lighthouse's runner fails websocket connections that work fine for real visitors — resolve the host yourself before "fixing" it. The audit still costs you the point either way.
- **Adding a third-party script? Default to `lazyOnload`.** Only GTM needs `afterInteractive`. Exception: anything that injects visible UI near the top of the page (e.g. Hello Bar), where deferring trades a JS win for a CLS loss.
- **Live is cPanel, not Vercel** — deploy is `git pull` + `npm run build` on the server. There is no dashboard redeploy button.
