...
// Runs at static-generation time. If this fires in the Vercel build logs, the
// endpoint env var isn't set for that build, so every WP section renders empty.
if (!process.env.NEXT_PUBLIC_WORDPRESS_GRAPHQL_ENDPOINT) {
  console.warn("[wp] NEXT_PUBLIC_WORDPRESS_GRAPHQL_ENDPOINT is not set — all WordPress data will be empty.");
}
...
export async function getCaseStudyNodes(ids, first = 6) {
  ...
  try {
    ...
    // Surfaces in Vercel build logs (these run at static-generation time).
    console.error("[wp] GraphQL fetch failed:", err?.message || err);
    return [];
  }
}
