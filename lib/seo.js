...
function cleanYoastSchema(raw) {
  ...
  try {
    ...
    if (Array.isArray(parsed["@graph"])) {
      // On the home page Yoast emits a degenerate one-crumb BreadcrumbList —
      // [{ position: 1, name: "Home" }] with no `item`, because it treats the last
      // crumb as the current page. Google's Rich Results Test reports that as a
      // critical 'Missing field "item"'. One crumb conveys no hierarchy and can
      // never produce a breadcrumb rich result, so drop the node rather than
      // invent an `item` for it. Deeper pages have 2+ crumbs where only the LAST
      // omits `item` (valid per Google) and are left untouched.
      const deadBreadcrumbIds = new Set(
        parsed["@graph"]
          .filter(
            (n) =>
              n["@type"] === "BreadcrumbList" &&
              (!Array.isArray(n.itemListElement) || n.itemListElement.length < 2)
          )
          .map((n) => n["@id"])
          .filter(Boolean)
      );
      ...
      parsed["@graph"] = parsed["@graph"]
        ...
        .filter((n) => !(n["@type"] === "BreadcrumbList" && deadBreadcrumbIds.has(n["@id"])))
        .map((n) => {
          ...
          // Don't leave a `breadcrumb` @id pointing at a node we just removed.
          if (n.breadcrumb?.["@id"] && deadBreadcrumbIds.has(n.breadcrumb["@id"])) {
            delete n.breadcrumb;
          }
          return n;
        });
