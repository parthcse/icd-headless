...
/**
 * Self-referencing canonical URL for a path.
 *
 * `getYoastMetadataByUri` already sets `alternates.canonical` for every
 * CMS-backed page. Routes that build their metadata by hand (team members, blog
 * pagination/category/author) don't go through it and would otherwise ship NO
 * canonical at all, so they call this instead.
 *
 * Usage: `return { title, description, alternates: { canonical: canonicalUrl(`/team/${slug}/`) } }`
 */
export function canonicalUrl(uri) {
  return `${PUBLIC_ORIGIN}${normPath(uri)}`;
}
    ...
    if (Array.isArray(parsed["@graph"])) {
      parsed["@graph"] = parsed["@graph"]
        ...
        .map((n) => {
          ...
          // Yoast omits `item` from the LAST crumb (it treats it as the current
          // page). On the home page that's the only crumb → "Missing field item";
          // on deeper pages the last crumb also omits it. Google *allows* the last
          // one to be item-less, but to keep every breadcrumb fully valid site-wide
          // we fill any missing `item` with the page's own URL (the breadcrumb @id
          // minus #breadcrumb). Applies to every BreadcrumbList, any length.
