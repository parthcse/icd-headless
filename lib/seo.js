...
import { SEO_BY_URI_QUERY, SEO_META_BY_URI_QUERY, CONTENT_TYPE_ARCHIVE_SEO_QUERY } from "../graphql/seoQueries";
...
export async function getYoastMetadataByUri(uri) {
  if (!process.env.NEXT_PUBLIC_WORDPRESS_GRAPHQL_ENDPOINT || !uri) return null;
  try {
    const { data } = await client.query({
    ...
    if (!seo || (!seo.title && !seo.metaDesc)) return null;
    ...
    if (seo.metaRobotsNoindex === "noindex") meta.robots = { index: false, follow: false };
