...
function mapNode(n) {
  return {
    id: n.databaseId,
    title: n.portfolioFields?.portfolioTitle || n.title,
    href: n.uri || "#", // internal path to the (future) single-portfolio page: /portfolio/<slug>/
    image: n.featuredImage?.node?.sourceUrl || "",
    imageAlt: n.featuredImage?.node?.altText || n.portfolioFields?.portfolioTitle || n.title,
...
export async function getPortfolioBySlug(slug) {
  ...
  try {
    ...
    return {
      title: f.portfolioTitle || p.title,
